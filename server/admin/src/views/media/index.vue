  ...
  <div class="media-container">
    <el-card>
      ...
      <div class="filter-bar">
        ...
        >
          <template #append>
            <el-button :icon="Search" @click="fetchMedia" />
          </template>
        </el-input>
      </div>
        ...
        >
          <div class="media-preview">
            ...
            <div v-else class="file-icon">
              <el-icon :size="60">
                <VideoPlay v-if="item.type === 'video'" />
                <Document v-else />
              </el-icon>
            </div>
          </div>
          ...
          <div class="media-actions">
            <el-button link type="primary" size="small" @click.stop="handleCopyUrl(item)">复制链接</el-button>
    ...
    <el-dialog v-model="previewVisible" title="预览" width="800px">
      <div v-if="currentMedia" class="preview-content">
        ...
        <div class="preview-info">
          ...
          <p v-if="currentMedia.width && currentMedia.height">
            <strong>尺寸:</strong> {{ currentMedia.width }}x{{ currentMedia.height }}
          </p>
...
const fetchMedia = async () => {
  try {
    const res = await getMediaListApi({
      ...
      type: filterType.value as MediaType,
