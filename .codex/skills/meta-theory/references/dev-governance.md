...
> For the theory source, see `.codex/skills/meta-theory/references/meta-theory.md`.
...
Optional Claude **Stop hook** (project default off): `META_KIM_STOP_COMPLETION_GUARD=hint` logs a stderr reminder when the last assistant message claims completion without governance cues; `=block` returns `{"decision":"block",...}` so the model continues. See `.codex/hooks/stop-completion-guard.mjs`.
...
```json
{
  ...
  "writebacks": [
    ...
    { "target": ".codex/skills/<skill>/SKILL.md", "reason": "reusable execution pattern" },
...
|--------------|-----------------|-----------|
| **Agent Boundary / CT / DR Adjustments** | `.codex/agents/{agent}.md` (direct edit) | Immediate; primary evolution target — triggers `npm run meta:sync` |
| **New Skills** (extracted) | `.codex/skills/{skill-name}/SKILL.md` | Permanent; created via skill-creator, validated via Type D Review |
...
**Card naming note**: English names are canonical in this repository. Use `.codex/skills/meta-theory/references/meta-theory.md` as the theory source and align wording with your audience and locale.
